...
std::vector<const DLTensor*> ExecutionEngine::inferOutputTensorInfo(
    ...
    auto ei = std::find_if(
        ...
        [&](const std::unique_ptr<ExecutorInfo>& ei) {
          return ei && name == ei->identifier &&
              dlutils::compareDLTensorVectorMetadata(
  ...
  auto execInfo = std::unique_ptr<ExecutorInfo>(new ExecutorInfo(
      name,
      inputs,
      std::unique_ptr<MappingOptions>(nullptr)->toProtobufSerializedString(),
      tcNameMap_.at(name),
